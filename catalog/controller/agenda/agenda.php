...
class ControllerAgendaAgenda extends Controller {
     ...
     * @param type $email
     */
    public function agendaSendMail($dados) {
        ...
        //destinatario
        $mail->AddAddress($dados['email'], $dados['email']);
        ...
        //remetente
        $adminMail->From = 'admin@wecareauto.com.br';
        $adminMail->FromName = 'Wecare';

        //dados do e-mail
        $adminMail->IsHTML(TRUE);
        $adminMail->CharSet = 'iso-8859-1';

        $adminMail->Subject = "Wecare - Agenda";
        $adminMail->Body = $message;
        
        //destinatario
        $adminMail->AddAddress('60gratisdewecareauto@gmail.com', '60gratisdewecareauto@gmail.com');
...
HTML;
        foreach ($dados as $key =>$value) {
            ...
            if($key == 'vencimento_cnh' || $key == 'vencimento_seguro' || $key == 'dt_ultima_revisao') {
                
                $adminMail->Body .= '<td>' . date('d/m/Y', $value) . '</td>';
            } else {
                
                $adminMail->Body .= '<td>' . iconv('UTF-8', 'ISO-8859-1', $value) . '</td>';
            }
        }
        $adminMail->Body .= <<<HTML
            </tbody>
